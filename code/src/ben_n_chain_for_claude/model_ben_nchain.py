...
class EpistemicNetwork(nn.Module):
    ...
    def __init__(self, state_dim: int, hidden_dim: int = 64, latent_dim: int = 8):
        super().__init__()
        self.hidden_dim = hidden_dim
        self.latent_dim = latent_dim
        self.state_dim = state_dim

        # Encoder network (from hidden state to latent)
        self.encoder = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, 2 * latent_dim),  # mu and sigma for each latent dim
        )

        # Decoder network (from latent to state)
        self.decoder = nn.Sequential(
            nn.Linear(latent_dim, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, state_dim),  # Reconstruct original state
        )
