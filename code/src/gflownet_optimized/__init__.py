...
from .train import GFlowNetTrainingConfig, train_gflownet, visualize_training_metrics
...
def create_random_env_factory(
    ...
    def is_valid_position(
        pos: Tuple[int, int], obstacles: List[Tuple[int, int]]
    ) -> bool:
    ...
    def has_valid_path(
        start: Tuple[int, int], goal: Tuple[int, int], obstacles: List[Tuple[int, int]]
    ) -> bool:
        ...
        while queue:
            ...
            for dx, dy in [(0, 1), (1, 0), (0, -1), (-1, 0)]:
                ...
                if is_valid_position(next_pos, obstacles) and next_pos not in visited:
    ...
    def generate_random_layout() -> (
        ...
        raise ValueError(
            f"Could not generate valid layout after {max_attempts} attempts"
        )
...
def test_random_env_factory():
    ...
    factory = create_random_env_factory(
        width=6, height=6, obstacle_density=0.2, min_path_length=4
    )
...
def main():
    ...
    # Train
    results, model = train_gflownet(curriculum, config)
    ...
    visualize_training_metrics(
        ...
        epsilon_history=results["epsilons"],
        temperature_history=results["temperatures"],
        success_history=results["successes"],
        success_rate_history=results["success_rates"],
        gflownet=model,
