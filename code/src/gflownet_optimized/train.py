...
def train_gflownet(
    ...
    buffer = PrioritizedReplayBuffer(
        ...
        alpha=0.6,
        beta=0.4,
        device=device,  # Pass device to buffer
    )
    ...
    # Create initial environment factory and environments
    ...
    # Initialize GFlowNet and move to GPU
    ...
    gflownet = GFlowNet(gflownet_config).to(device)
    ...
    for episode in range(config.num_episodes):
        # Collect trajectories with GPU acceleration
        trajectories = collect_trajectories_batch(envs, gflownet, epsilon, temperature)
        ...
        if curriculum.should_increase_difficulty():
            ...
            print(
                f"Episode {episode}: Increasing difficulty to {curriculum.get_current_params()}"
            )
        ...
        if len(buffer) >= config.min_experiences:
            ...
            if result is not None:
                ...
                # Update with mixed precision if using GPU
                with torch.amp.autocast(
                    device_type=device.type, enabled=device.type == "cuda"
                ):
                    metrics = gflownet.update(
                        train_trajectories, indices=indices, weights=weights
                    )

                # Update buffer priorities
                if hasattr(buffer, "update_priorities"):
                    # Create errors tensor with same batch size as indices
                    errors = torch.full(
                        (len(indices),), metrics["td_errors"], device=device
                    )
        ...
        temperature = max(
            config.temperature_end, temperature * config.temperature_decay
        )

        epsilon_history.append(epsilon)
        temperature_history.append(temperature)

        # Optional: Empty CUDA cache periodically
...
def collect_trajectories_batch(
    ...
    """Collect multiple trajectories with optimized GPU usage."""
    device = gflownet.device
    batch_size = gflownet.config.batch_size

    # Initialize trajectories on device
    trajectories = [
        Trajectory([], [], [], [], False, 0.0, 0) for _ in range(batch_size)
    ]

    # Pre-allocate tensors for batch processing
    active_envs = list(range(batch_size))
    states_tensor = torch.zeros((batch_size, envs[0].state_dim), device=device)
    masks_tensor = torch.ones(batch_size, device=device, dtype=torch.bool)

    # Get initial states - move to device once
    current_steps = [env.reset() for env in envs]
    for i, step in enumerate(current_steps):
        states_tensor[i] = step.state

    while any(masks_tensor):
        active_indices = masks_tensor.nonzero().squeeze(-1)
        if len(active_indices.shape) == 0:
            active_indices = active_indices.unsqueeze(0)

        active_states = states_tensor[active_indices]
        active_count = len(active_indices)

        # Compute exploration decisions - all on GPU
        random_actions = torch.rand(active_count, device=device) < epsilon

        # Get policy actions
        with torch.amp.autocast(device_type=device.type, enabled=True):
            output = gflownet.forward(active_states)
            logits = output.logits / temperature
            policy_probs = F.softmax(logits, dim=-1)
            policy_actions = torch.multinomial(policy_probs, 1).squeeze(-1)

        # Random actions
        random_action_indices = torch.randint(
            gflownet.config.action_dim, (active_count,), device=device
        )

        # Combine actions
        actions = torch.where(random_actions, random_action_indices, policy_actions)

        # Update trajectories and get next states - minimize CPU transfers
        new_states = torch.zeros_like(states_tensor)
        new_masks = torch.zeros_like(masks_tensor)
        rewards = torch.zeros(batch_size, device=device)

        # Process environment steps in batch
        for batch_idx, env_idx in enumerate(active_indices.cpu().numpy()):
            action = Action(actions[batch_idx].cpu().item())
            step_data = envs[env_idx].step(action)

            # Update trajectory on device
            trajectories[env_idx].states.append(states_tensor[env_idx].clone())
            trajectories[env_idx].action_indices.append(actions[batch_idx].clone())
            trajectories[env_idx].actions.append(action)
            trajectories[env_idx].rewards.append(step_data.reward.to(device))
            trajectories[env_idx].total_reward += step_data.reward.item()
            trajectories[env_idx].length += 1

            if not step_data.done:
                new_states[env_idx] = step_data.state
                new_masks[env_idx] = 1
            else:
...
def visualize_training_metrics(
    ...
    success_avg = np.convolve(
        success_history, np.ones(window_size) / window_size, mode="valid"
    )
    rate_avg = np.convolve(
        success_rate_history, np.ones(window_size) / window_size, mode="valid"
    )
    ax2.plot(
        range(window_size - 1, len(success_history)),
        success_avg,
        color="darkgreen",
        label="Success MA",
    )
    ax2.plot(
        range(window_size - 1, len(success_rate_history)),
        rate_avg,
        color="darkblue",
        label="Rate MA",
    )
...
#     trajectories = [
#         Trajectory([], [], [], [], False, 0.0, 0)
#         for _ in range(gflownet.config.batch_size)
#     ]
...
#             random_action_indices = torch.randint(
#                 gflownet.config.action_dim, (len(states),)
#             )

#             # Combine random and policy actions
#             action_indices = torch.where(
#                 random_mask, random_action_indices, policy_actions
#             )
