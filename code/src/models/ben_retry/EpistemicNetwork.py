...
def construct_epsitemic_flow(
    ...
    flows_epistemic = []
    prior_phi = nf.distributions.DiagGaussian(
        [1, 1, num_params_aleatoric], trainable=False
    ).to(device)
    prior_phi.log_scale = torch.nn.Parameter(torch.log(torch.tensor(0.1))).to(device)
    base_epistemic_dist = nf.distributions.DiagGaussian(
        [1, 1, num_params_aleatoric], trainable=False
    ).to(device)
    ...
    for _ in range(num_layers):
        ...
        flows_epistemic.append(
            LULinearPermute(num_channels=num_params_aleatoric, identity_init=True).to(
                device
            )
        )
