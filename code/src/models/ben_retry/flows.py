...
class AbsFlow(nf.flows.Flow):
    ...
    def __init__(self, K, conditioner_input_dim=2, hidden_dim=2, device="cuda:0"):
        ...
        for i in range(K):
            self.conditioners.append(
                ConditionerMLP(conditioner_input_dim, hidden_dim, 2, device=device).to(
                    device
                )
            )
    ...
    def inverse(
        ...
        conditioner_input, _ = torch.split(
            params, [x.shape[-1] + 1, params[0, 0, 0, 1:].numel() - x.shape[-1]], dim=-1
        )
        xi = x
        log_det = torch.zeros_like(x)
...
class ConditionalReverse(Flow):
    ...
    def forward(
        ...
        return self.flow.inverse(
            z, params, conditioner_network, q_network, inputs_for_q, hidden_state
        )
...
class LULinearPermute(Flow):
    ...
    def inverse(self, z):
        ...
        batch_dims_x = z.shape[1]
        log_det = torch.zeros(
            (batch_dims_phi, batch_dims_x, z.shape[-2], z.shape[-1]), dtype=torch.double
        )
...
class MaskedLinear(nn.Linear):
    ...
    def forward(
        ...
        weight = weight.reshape(
            weight.shape[0], weight.shape[1], self.weight.shape[1], self.weight.shape[0]
        )
        weight.retain_grad()
        assert x.dim() == 4
...
class MaskedResidualBlock(nn.Module):
    ...
    def forward(
        ...
        temps = self.linear_layers[0](
            temps, params, conditioner_net, q_network, inputs_for_q, hidden_state
        )
        if self.use_batch_norm:
            temps = self.batch_norm_layers[1](temps)
        temps = self.activation(temps)
        temps = self.dropout(temps)
        temps = self.linear_layers[1](
            temps, params, conditioner_net, q_network, inputs_for_q, hidden_state
        )
        if context is not None:
...
class MADE(nn.Module):
    ...
    def forward(
        ...
        for block in self.blocks:
            outputs = block(
                outputs, params, conditioner_net, q_network, inputs_for_q, hidden_state
            )
