...
class GFlowNet(nn.Module):
    def __init__(self, num_hidden: int = 128):
        ...
        self.state_log_flow = nn.Linear(num_hidden, 1)
...
def trainer(
    ...
    for episode in range(num_episodes):
        ...
        # # Prevent gradients from becoming too large, which can cause unstable updates.
        # max_norm = 1.0
        # torch.nn.utils.clip_grad_norm_(flow_network.parameters(), max_norm)
...
def compute_flow_loss(trajectory, flow_network, env):
    ...
    for t in range(len(trajectory)):
        ...
        log_policy_s, log_flow_s = flow_network(state_tensor)
        log_policy_s_prime, log_flow_s_prime = flow_network(next_state_tensor)

        # Get the log probability of the taken action
        log_p_forward = log_policy_s[0, action]
        ...
        if R_s_prime > 0:
            ...
            # Compute log F_out(s') = R(s') + sum(log_F(s', a'), a' in A(s'))
            # _, next_action, _ = trajectory[t + 1]
            # log_F_out_s_prime = log_R_s_prime + log_flow_s_prime + log_probs_s_prime[0, next_action]
            log_F_out_s_prime = log_R_s_prime + log_flow_s_prime + sum(log_policy_s_prime[0, a_prime] for a_prime in [0, 1, 2, 3])

            log_F_out_s_prime = torch.logsumexp(torch.stack([log_R_s_prime, log_F_in_s_prime]), dim=0)
            log_diff = log_F_in_s_prime - log_F_out_s_prime
        else:
            log_diff = log_F_in_s_prime
...
if __name__ == "__main__":
    ...
    # Train the GFlowNet
    world.run_training(lambda w: trainer(w, show_policy=True))
