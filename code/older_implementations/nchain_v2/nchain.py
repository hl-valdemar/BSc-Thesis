...
class NChainEnv:
    ...
    Attributes:
        n: Length of the chain
        base_reward: Base from which to calculate reward
        path_penalty: The penalty attributed path length
    """
    ...
    def __init__(
        ...
        Args:
            ...
            base_reward: Base from which to calculate reward
            path_penalty: The penalty attributed path length
            efficiency_threshold: The efficiency level to strive for
        """
        self.n = n
    ...
    def step(self, action: int) -> Tuple[NChainState, float, bool]:
        ...
        # If terminal state, give reward (incorporates path length penalty)
        # This effectively creates new terminal states for each possible path length
        if done:
            reward = self.base_reward * (self.path_penalty**self.steps_taken)
        else:
            reward = 0.0
