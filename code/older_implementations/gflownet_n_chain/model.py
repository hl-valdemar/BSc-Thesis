...
from n_chain import Action, NChain
...
class GFlowNet(nn.Module):
    ...
    def __init__(self, n_states: int, n_actions: int, hidden_dim: int = 64):
        ...
        self.flow_net = nn.Sequential(
            ...
            nn.Softplus(),  # Ensures positive flows: softplus(x) = log(1 + exp(x))
        )
    ...
    def forward(self, states: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Compute flows (already positive due to softplus) and convert to log-space
        flows: torch.Tensor = self.flow_net(x)  # [batch_size, 1]
        log_flows = torch.log(flows + 1e-8)  # Add small epsilon for numerical stability

        return policy_logits, log_flows

    def compute_trajectory_balance_loss(
        self,
        trajectories: List[List[Tuple[int, int]]],  # List of (state, action) pairs
        rewards: torch.Tensor,  # Terminal rewards for each trajectory [batch_size]
    ) -> torch.Tensor:
        """
        Shape guide:
        - state_tensor: [batch_size, max_len]
        - action_tensor: [batch_size, max_len]
        - mask: [batch_size, max_len]
        - policy_logits: [batch_size, n_actions]
        - log_probs: [batch_size, n_actions]
        - step_log_probs: [batch_size, 1]
        - total_log_probs: [batch_size]
        - initial_log_flows: [batch_size, 1]
        """
        """
        ...
        assert rewards.shape == (
            batch_size,
        ), f"Expected rewards shape (batch_size,), got {rewards.shape}"
        ...
        # Initialize total_log_probs with correct shape [batch_size]
        ...
        for t in range(max_len):
            ...
            # Get policy logits for current states [batch_size, n_actions]
            policy_logits, _ = self.forward(state_tensor[:, t])

            # Compute log probabilities of chosen actions
            log_probs = F.log_softmax(policy_logits, dim=-1)  # [batch_size, n_actions]
            step_log_probs = torch.gather(
                log_probs,
                1,
                action_tensor[:, t].unsqueeze(-1),  # [batch_size, 1]
            ).squeeze(-1)  # [batch_size]

            # Add to total, considering mask [batch_size]
            total_log_probs += step_log_probs * mask[:, t]

        # Compute loss: (log F(s₀) + Σ log PF(aₜ|sₜ) - log R(x))²
        log_rewards = torch.log(rewards + 1e-8)  # [batch_size]
        loss = (
